...
class start_game:
    ...
    def start_all(self):  ##開始鍵的function
        ...
        song = {'A': r"C:\Users\user\Desktop\Merry\音樂健康\0512\Song1\song1.wav", 'B': r"C:\Users\user\Desktop\Merry\音樂健康\0512\Song2\song2.wav"}  ##音樂dict
        self.set.restarted = True

        self.set.camera_cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)
        if self.ui.stackedWidget.currentIndex() ==3: ##第三頁關卡為yellow(可以直接更改)
            music_path = r"C:\Users\user\Desktop\Merry\音樂健康\music\Yellow.mp3"
            self.set.choose_song = ""
        if self.ui.stackedWidget.currentIndex() == 1:
            music_path = song[self.set.choose_song]        ##選擇的對應音樂
        
        self.set.camera_cap.set(cv2.CAP_PROP_FRAME_WIDTH, 320)  ###鏡頭參數set
    ...
    def restart(self):  ##restart鍵的function(重製一些基本參數，回到第一頁)
        ...
        threading.Thread(target=self.set.main_app.checking_mode, daemon=True).start()  ###確認所選擇的歌曲(回到Main的chacking mode)
    
    def start_score_timer(self):  ##開始計分
